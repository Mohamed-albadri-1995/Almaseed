...
/**
 * Rich WYSIWYG editor for written articles.
 * The editor is also used inside the mobile app because the contributor/admin
 * pages are opened in the app's WebView, so keep the UI touch-friendly and
 * selection-safe on both desktop and mobile browsers.
 */
...
function Btn({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`flex h-9 min-w-9 shrink-0 touch-manipulation items-center justify-center rounded-md px-2 text-sm font-bold transition ${
...
export function ArticleEditor({
  ...
  return (
    <div className="overflow-visible rounded-2xl border border-ivory-300 bg-white">
      <div className="sticky top-[80px] z-30 flex min-h-12 flex-nowrap items-center gap-1 overflow-x-auto overscroll-contain rounded-t-2xl border-b border-ivory-200 bg-ivory-50 p-1.5 shadow-sm [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <select
          ...
          className="h-9 shrink-0 rounded-md border border-ivory-300 bg-white px-2 text-sm text-brand-800"
          defaultValue=""
        >
        ...
        {COLORS.map((c) => (
          <button
            ...
            className="h-7 w-7 shrink-0 touch-manipulation rounded-full border border-black/10"
      ...
      <div className="relative">
        ...
        <div
          ...
          className="article-prose block w-full px-4 py-5 text-[16px] leading-8 outline-none sm:px-6 sm:text-base"
          style={{ minHeight }}
        />
      </div>

      <p className="border-t border-ivory-200 bg-ivory-50/60 px-4 py-2 text-xs text-muted">
        حرّر النص مباشرةً كما سيظهر للقارئ — شريط الأدوات يبقى متاحًا أثناء الكتابة والتمرير.
      </p>
    </div>
  );
}
