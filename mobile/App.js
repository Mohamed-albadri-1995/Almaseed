...
function SeekBar({ position, duration, onSeek }) {
  ...
  // The app forces global RTL, and Android IGNORES `direction:'ltr'` on a single
  // view — so this bar is mirrored by the platform: the fill/thumb (left:%) render
  // from the RIGHT edge and the elapsed time sits on the right. Touch coordinates,
  // however, stay physical (locationX from the LEFT). So we mirror the touch with
  // 1-raw to match the mirrored layout: this makes the thumb follow the finger
  // exactly and dragging toward the right edge goes to the start (natural in RTL).
  const fractionFromEvent = (e) => {
    const raw = (e.nativeEvent.locationX || 0) / wRef.current;
    return clamp(I18nManager.isRTL ? 1 - raw : raw);
  };
