...
function SeekBar({ position, duration, onSeek }) {
  ...
  // The seek subtree is forced to LTR (styles.seekWrap → direction:'ltr'), so it
  // is NOT mirrored by the app's global RTL. That keeps everything physical and
  // consistent: the fill/thumb are positioned from the left edge and locationX is
  // measured from the left edge too, so the raw ratio maps directly with no RTL
  // flip. (This is the reference build's proven approach — do not add a 1-raw
  // mirror here; that double-compensates and reverses the drag.)
