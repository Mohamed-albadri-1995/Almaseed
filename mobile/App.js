...
export default function App() {
  ...
  const enterPip = useCallback((data) => { setPip(data); setStack((s) => (s.length > 1 ? s.slice(0, -1) : s)); }, []);
  const expandPip = useCallback(() => { setPip((p) => { if (p) setStack((s) => [...s, { name: 'material', params: { id: p.id, resumeMs: p.positionMillis } }]); return null; }); }, []);
...
function MaterialScreen({ id, onBack, onPlay, onStop, onPip, resumeMs, nowId }) {
  ...
  return <View style={{ flex: 1 }}><Header title="تفاصيل المادة" onBack={onBack} />{err ? <ErrorBox msg={err} /> : !m ? <Loader /> : <ScrollView contentContainerStyle={{ padding: 16 }}><Text style={styles.detailTitle}>{m.title}</Text>{!!m.subtitle && <Text style={styles.detailSub}>{m.subtitle}</Text>}{!!(m.performer || m.speaker || m.host) && <Text style={styles.detailPerson}>{m.performer || m.speaker || m.host}</Text>}{m.fileKind === 'IMAGE' && m.fileUrl ? <Image source={{ uri: m.fileUrl }} style={styles.image} resizeMode="contain" /> : m.fileKind === 'VIDEO' && m.fileUrl ? <InlineVideo url={m.fileUrl} poster={m.coverImage} resumeMs={resumeMs} onMinimize={(positionMillis) => onPip && onPip({ id: m.id, url: m.fileUrl, poster: m.coverImage, title: m.title, positionMillis })} /> : m.fileKind === 'AUDIO' && m.fileUrl ? (playingHere ? <FullAudioPlayer title={m.title} person={m.performer || m.speaker || m.host || null} poster={m.coverImage} onStop={onStop} /> : <TouchableOpacity style={styles.playCard} onPress={() => onPlay(m)} activeOpacity={0.9}>{m.coverImage ? <Image source={{ uri: m.coverImage }} style={StyleSheet.absoluteFill} resizeMode="cover" /> : null}<View style={styles.playCardOverlay}><View style={styles.playCircle}><Text style={styles.playCircleIcon}>▶</Text></View><Text style={styles.playCardLabel}>استماع</Text><Text style={styles.playCardHint}>يستمر التشغيل أثناء تصفّح باقي الصفحات</Text></View></TouchableOpacity>) : null}{!!m.bodyText && <View style={styles.article}><Text style={styles.articleText}>{m.bodyText}</Text></View>}{!!m.description && <Text style={styles.desc}>{m.description}</Text>}<Downloads material={m} /></ScrollView>}</View>;
}
function InlineVideo({ url, poster, resumeMs, onMinimize }) {
  const ref = useRef(null); const posRef = useRef(0); const [err, setErr] = useState(false);
  const fullscreen = async () => { try { await ref.current?.presentFullscreenPlayer(); } catch {} };
  return <View style={styles.videoWrap}><Video ref={ref} source={{ uri: url }} useNativeControls resizeMode={ResizeMode.CONTAIN} usePoster={!!poster} posterSource={poster ? { uri: poster } : undefined} style={styles.videoInline} onError={() => setErr(true)} onPlaybackStatusUpdate={(s) => { if (s && s.positionMillis) posRef.current = s.positionMillis; }} onLoad={() => { if (resumeMs) { ref.current?.setPositionAsync(resumeMs); ref.current?.playAsync(); } }} />{err ? <Text style={styles.videoErr}>تعذّر تشغيل الفيديو — جرّب التنزيل.</Text> : <View style={styles.videoBtns}>{onMinimize && <TouchableOpacity style={styles.fsBtn} onPress={() => onMinimize(posRef.current)} activeOpacity={0.85}><Text style={styles.fsIcon}>⤵</Text><Text style={styles.fsTxt}>تصغير</Text></TouchableOpacity>}<TouchableOpacity style={[styles.fsBtn, styles.fsBtnAlt]} onPress={fullscreen} activeOpacity={0.85}><Text style={styles.fsIcon}>⛶</Text><Text style={styles.fsTxt}>ملء الشاشة</Text></TouchableOpacity></View>}</View>;
}
function PipVideo({ item, onExpand, onClose }) {
  const ref = useRef(null); const { width: W, height: H } = Dimensions.get('window'); const pan = useRef(new Animated.ValueXY({ x: W - 178, y: H - 220 })).current;
  const responder = useRef(PanResponder.create({ onMoveShouldSetPanResponder: (_e, g) => Math.abs(g.dx) > 4 || Math.abs(g.dy) > 4, onPanResponderGrant: () => { pan.setOffset({ x: pan.x.__getValue(), y: pan.y.__getValue() }); pan.setValue({ x: 0, y: 0 }); }, onPanResponderMove: Animated.event([null, { dx: pan.x, dy: pan.y }], { useNativeDriver: false }), onPanResponderRelease: () => pan.flattenOffset() })).current;
  return <Animated.View style={[styles.pip, { transform: pan.getTranslateTransform() }]} {...responder.panHandlers}><Video ref={ref} source={{ uri: item.url }} resizeMode={ResizeMode.CONTAIN} shouldPlay style={styles.pipVideo} onLoad={() => { if (item.positionMillis) ref.current?.setPositionAsync(item.positionMillis); }} /><TouchableOpacity style={styles.pipTap} activeOpacity={0.9} onPress={onExpand} /><View style={styles.pipBar}><TouchableOpacity onPress={onExpand} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}><Text style={styles.pipIcon}>⤢</Text></TouchableOpacity><TouchableOpacity onPress={onClose} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}><Text style={styles.pipIcon}>✕</Text></TouchableOpacity></View></Animated.View>;
}
function MiniPlayer(props) { return props.item.fileKind === 'VIDEO' ? <MiniVideo {...props} /> : <MiniAudio {...props} />; }
...
function SeekBar({ position, duration, onSeek }) {
  ...
  return <View><View {...pan.panHandlers} onLayout={(e) => { wRef.current = e.nativeEvent.layout.width || 1; }} style={styles.seekHit}><View style={styles.seekTrack}><View style={[styles.seekFill, { width: `${frac * 100}%` }]} /><View style={[styles.seekThumb, { left: `${frac * 100}%` }]} /></View></View><View style={styles.seekTimes}><Text style={styles.seekTime}>{fmtTime(duration)}</Text><Text style={styles.seekTime}>{fmtTime(drag != null ? drag * duration : position)}</Text></View></View>;
}
...
function MiniVideo({ item, onClose, onOpen }) { const videoRef = useRef(null); const [status, setStatus] = useState({ isPlaying: false, positionMillis: 0, durationMillis: 1 }); const toggle = async () => { try { if (status.isPlaying) await videoRef.current?.pauseAsync(); else await videoRef.current?.playAsync(); } catch {} }; const pct = status.durationMillis ? Math.min(100, Math.round((status.positionMillis / status.durationMillis) * 100)) : 0; return <MiniShell item={item} onClose={onClose} onOpen={onOpen} pct={pct} leading={<TouchableOpacity onPress={onOpen} activeOpacity={0.9}><Video ref={videoRef} source={{ uri: item.fileUrl }} resizeMode={ResizeMode.COVER} shouldPlay onPlaybackStatusUpdate={(s) => setStatus(s)} style={styles.miniVideo} /></TouchableOpacity>}><TouchableOpacity onPress={toggle} style={styles.miniBtn} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}><Text style={styles.miniBtnIcon}>{status.isPlaying ? '❚❚' : '▶'}</Text></TouchableOpacity></MiniShell>; }
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: C.brand, paddingBottom: 12, paddingHorizontal: 12 }, headerTitle: { color: C.white, fontSize: 17, fontWeight: '800', flex: 1, textAlign: 'center' }, backBtn: { width: 92, flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: '#ffffff22', borderRadius: 20, paddingVertical: 8, paddingHorizontal: 12 }, backChevron: { color: C.white, fontSize: 20, fontWeight: '900', lineHeight: 22, marginTop: -2 }, backTxt: { color: C.white, fontSize: 15, fontWeight: '800' },
