...
const KIND_ICON = { AUDIO: 'mic', VIDEO: 'videocam', IMAGE: 'image', DOCUMENT: 'document-text', ARTICLE: 'document-text' };
function KindIcon({ kind, size = 34, color = C.gold300 }) { return <Ionicons name={KIND_ICON[kind] || 'musical-note'} size={size} color={color} />; }
...
function Downloads({ material }) { const [busy, setBusy] = useState(''); const [pct, setPct] = useState(0); const ext = (material.fileType || (material.bodyText ? 'txt' : 'dat')).toLowerCase(); const safe = material.title.replace(/[^\p{L}\p{N} _-]/gu, '').slice(0, 40) || 'material'; const filename = `${safe}-${material.id}.${ext}`; const MIME = { pdf: 'application/pdf', doc: 'application/msword', docx: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', txt: 'text/plain', jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', webp: 'image/webp' }; async function ensureLocal(onProgress) { const dest = FileSystem.documentDirectory + filename; const info = await FileSystem.getInfoAsync(dest); if (info.exists) return dest; if (material.fileUrl) { const task = FileSystem.createDownloadResumable(material.fileUrl, dest, {}, (p) => { if (onProgress && p.totalBytesExpectedToWrite > 0) onProgress(Math.min(100, Math.round((p.totalBytesWritten / p.totalBytesExpectedToWrite) * 100))); }); const dl = await task.downloadAsync(); return dl.uri; } await FileSystem.writeAsStringAsync(dest, material.bodyText || material.description || ''); return dest; } const saveInApp = async () => { try { setBusy('app'); setPct(0); const localPath = await ensureLocal(setPct); await addDownload({ id: material.id, title: material.title, subtitle: material.subtitle || null, person: material.category?.slug === 'readings' ? material.author || null : (material.performer || material.speaker || material.host || null), fileKind: material.fileKind || (material.bodyText ? 'ARTICLE' : 'AUDIO'), localPath, bodyText: material.bodyText || null }); Alert.alert('تم الحفظ', 'حُفظت المادة داخل التطبيق، وتظهر في «التنزيلات المحفوظة».'); } catch (e) { Alert.alert('تعذّر الحفظ', String(e.message || e)); } finally { setBusy(''); } }; const saveToDevice = async () => { try { setBusy('device'); setPct(0); const isMedia = ['AUDIO', 'VIDEO', 'IMAGE'].includes(material.fileKind); const uri = await ensureLocal(setPct); if (isMedia) { const perm = await MediaLibrary.requestPermissionsAsync(false); if (!perm.granted) { Alert.alert('الإذن مطلوب', 'فعّل إذن الوسائط من إعدادات التطبيق لحفظ الملف في جهازك.'); return; } await MediaLibrary.saveToLibraryAsync(uri); Alert.alert('تم التنزيل', 'حُفظ الملف في جهازك (المعرض / الموسيقى).'); } else { const SAF = FileSystem.StorageAccessFramework; if (Platform.OS === 'android' && SAF) { const perm = await SAF.requestDirectoryPermissionsAsync(); if (!perm.granted) { Alert.alert('الإذن مطلوب', 'اختر مجلدًا لحفظ الملف في جهازك.'); return; } const b64 = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 }); const target = await SAF.createFileAsync(perm.directoryUri, safe, MIME[ext] || 'application/octet-stream'); await FileSystem.writeAsStringAsync(target, b64, { encoding: FileSystem.EncodingType.Base64 }); Alert.alert('تم التنزيل', 'حُفظ الملف في المجلد الذي اخترته.'); } else if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(uri); else if (material.fileUrl) Linking.openURL(material.fileUrl); else Alert.alert('غير متاح', 'تعذّر حفظ هذا النوع على الجهاز.'); } } catch (e) { Alert.alert('تعذّر التنزيل', String(e.message || e)); } finally { setBusy(''); } }; if (!material.fileUrl && !material.bodyText) return null; return <View style={styles.downloads}><TouchableOpacity style={styles.dlBtn} onPress={saveInApp} disabled={!!busy}><Text style={styles.dlTxt}>{busy === 'app' ? `${pct}%` : 'حفظ داخل التطبيق'}</Text></TouchableOpacity><TouchableOpacity style={[styles.dlBtn, styles.dlBtnAlt]} onPress={saveToDevice} disabled={!!busy}><Text style={[styles.dlTxt, { color: C.brand }]}>{busy === 'device' ? `${pct}%` : 'تنزيل إلى الجهاز'}</Text></TouchableOpacity></View>; }
