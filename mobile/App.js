...
function Downloads({ material }) { const [busy, setBusy] = useState(''); const [pct, setPct] = useState(0);
  ...
  // Write the downloaded file into the chosen SAF folder. createFileAsync makes
  // the document (name WITHOUT extension — SAF appends it from the MIME type)
  // and we stream the bytes in via base64. copyAsync cannot target a content://
  // destination (it calls toFile() on it and throws), so this is the only path
  // that actually writes into a picked folder.
