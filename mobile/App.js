...
import { getDownloads, addDownload, removeDownload, getNotifSeen, setNotifSeen, getAuth, setAuth, clearAuth, getSafDir, setSafDir } from './storage';
...
function Feed({ push, active, setActive, kind, setKind, q, setQ }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <Text style={styles.filterCap}>النوع</Text>
...
function SeekBar({ position, duration, onSeek }) {
  ...
  const elapsed = <Text style={styles.seekTime}>{fmtTime(drag != null ? drag * duration : position)}</Text>;
  const total = <Text style={styles.seekTime}>{fmtTime(duration)}</Text>;
  return <View style={styles.seekWrap}>
    ...
    >
      <View style={styles.seekTrack}><View style={[styles.seekFill, { ...anchorPct(0), width: `${frac * 100}%` }]} /><View style={[styles.seekThumb, anchorPct(frac * 100), isRTL ? { marginRight: -10 } : { marginLeft: -10 }]} /></View>
    </View>
    <View style={styles.seekTimes}>{isRTL ? <>{total}{elapsed}</> : <>{elapsed}{total}</>}</View>
  </View>;
}
...
function Downloads({ material }) { const [busy, setBusy] = useState(''); const [pct, setPct] = useState(0);
  ...
  const safe = material.title.replace(/[^\p{L}\p{N} _-]/gu, '').slice(0, 40) || 'material'; const displayBase = `${safe} - أرشيف المسيد`; const filename = `${material.id}.${ext}`; const MIME = { pdf: 'application/pdf', doc: 'application/msword', docx: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', txt: 'text/plain', jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', webp: 'image/webp', mp4: 'video/mp4', m4v: 'video/mp4', mov: 'video/quicktime', webm: 'video/webm', mkv: 'video/x-matroska', '3gp': 'video/3gpp', '3gpp': 'video/3gpp', avi: 'video/x-msvideo', mp3: 'audio/mpeg', m4a: 'audio/mp4', aac: 'audio/aac', wav: 'audio/wav', ogg: 'audio/ogg', oga: 'audio/ogg', opus: 'audio/ogg', amr: 'audio/amr', weba: 'audio/webm' }; async function ensureLocal(onProgress) { const dest = FileSystem.documentDirectory + filename; const info = await FileSystem.getInfoAsync(dest); if (info.exists) return dest; if (material.fileUrl) { const task = FileSystem.createDownloadResumable(material.fileUrl, dest, {}, (p) => { if (onProgress && p.totalBytesExpectedToWrite > 0) onProgress(Math.min(100, Math.round((p.totalBytesWritten / p.totalBytesExpectedToWrite) * 100))); }); const dl = await task.downloadAsync(); return dl.uri; } await FileSystem.writeAsStringAsync(dest, material.bodyText || material.description || ''); return dest; } const saveInApp = async () => { try { setBusy('app'); setPct(0); const localPath = await ensureLocal(setPct); await addDownload({ id: material.id, title: material.title, subtitle: material.subtitle || null, person: material.category?.slug === 'readings' ? material.author || null : (material.performer || material.speaker || material.host || null), fileKind: material.fileKind || (material.bodyText ? 'ARTICLE' : 'AUDIO'), localPath, bodyText: material.bodyText || null }); Alert.alert('تم الحفظ', 'حُفظت المادة داخل التطبيق، وتظهر في «التنزيلات المحفوظة».'); } catch (e) { Alert.alert('تعذّر الحفظ', String(e.message || e)); } finally { setBusy(''); } }; const SAF = FileSystem.StorageAccessFramework; const mimeOf = () => MIME[ext] || 'application/octet-stream';
  // Resolve the download folder the user picked. We remember it (SAF tree URI)
  // so downloads go there directly without re-prompting; `forcePick` re-opens
  // the system folder picker (e.g. after the permission was revoked).
  const pickSafDir = async (forcePick) => { if (Platform.OS !== 'android' || !SAF || typeof SAF.requestDirectoryPermissionsAsync !== 'function') return null; if (!forcePick) { const saved = await getSafDir(); if (saved) return saved; } const perm = await SAF.requestDirectoryPermissionsAsync(); if (!perm || !perm.granted) return null; await setSafDir(perm.directoryUri); return perm.directoryUri; };
  // Write a (small) document into the chosen SAF folder. createFileAsync makes
  // the file (name WITHOUT extension — SAF appends it from the MIME type) and we
  // copy the bytes in via base64. This is base64-in-memory, so it is used ONLY
  // for documents/articles — never for large media, which would freeze the app.
  // (copyAsync cannot target a content:// destination — it calls toFile() on it
  // and throws — so createFileAsync is the only way to write into a picked folder.)
  const writeToSaf = async (dirUri, uri) => { const destUri = await SAF.createFileAsync(dirUri, displayBase, mimeOf()); const b64 = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 }); await FileSystem.writeAsStringAsync(destUri, b64, { encoding: FileSystem.EncodingType.Base64 }); return true; };
  // Fallbacks used only when the folder write is impossible (non-Android, or an
  // error such as a very large file exhausting memory): keep the media in the
  // gallery/Music for media kinds, else offer the system share sheet.
  const saveViaShare = async (uri) => { try { let shareUri = uri; try { const pretty = FileSystem.cacheDirectory + `${displayBase}.${ext}`; await FileSystem.deleteAsync(pretty, { idempotent: true }); await FileSystem.copyAsync({ from: uri, to: pretty }); shareUri = pretty; } catch {} if (await Sharing.isAvailableAsync()) { await Sharing.shareAsync(shareUri); return true; } if (material.fileUrl) { Linking.openURL(material.fileUrl); return true; } } catch {} return false; };
  const saveToGallery = async (uri) => { if (!['AUDIO', 'VIDEO', 'IMAGE'].includes(material.fileKind)) return false; try { const perm = await MediaLibrary.requestPermissionsAsync(false); if (!perm.granted) return false; let saveUri = uri; try { const pretty = FileSystem.cacheDirectory + `${displayBase}.${ext}`; await FileSystem.deleteAsync(pretty, { idempotent: true }); await FileSystem.copyAsync({ from: uri, to: pretty }); saveUri = pretty; } catch {} await MediaLibrary.saveToLibraryAsync(saveUri); return true; } catch { return false; } };
  const saveToDevice = async () => { try { setBusy('device'); setPct(0); const uri = await ensureLocal(setPct); const isMedia = ['AUDIO', 'VIDEO', 'IMAGE'].includes(material.fileKind);
    // MEDIA (the bulk of the archive) is saved NATIVELY via MediaLibrary. This is
    // essential: writing into a SAF-picked folder needs a full base64 copy of the
    // file through the JS bridge, which freezes the app (ANR) and can run out of
    // memory on big audio/video. MediaLibrary streams it natively — fast, no freeze.
    if (isMedia) { if (await saveToGallery(uri)) { Alert.alert('تم التنزيل', 'حُفظ الملف في جهازك (المعرض / الموسيقى).'); return; } if (await saveViaShare(uri)) return; Alert.alert('الإذن مطلوب', 'فعّل إذن الوسائط من إعدادات التطبيق لحفظ الملف في جهازك.'); return; }
    // Documents/articles are small, so a picked-folder write is safe here.
    let dir = await pickSafDir(false); if (!dir) { if (await saveViaShare(uri)) return; Alert.alert('لم يُختر مجلد', 'اختر مجلدًا لحفظ الملف في جهازك.'); return; }
    try { await writeToSaf(dir, uri); Alert.alert('تم التنزيل', 'حُفظ الملف في المجلد الذي اخترته.'); return; } catch (e) { let dir2 = null; try { dir2 = await pickSafDir(true); } catch {} if (dir2) { try { await writeToSaf(dir2, uri); Alert.alert('تم التنزيل', 'حُفظ الملف في المجلد الذي اخترته.'); return; } catch {} } if (await saveViaShare(uri)) return; throw e; } } catch (e) { Alert.alert('تعذّر التنزيل', String(e.message || e)); } finally { setBusy(''); } }; const shareLink = async () => { try { await Share.share({ message: `${material.title}\n${API_BASE}/material/${material.id}` }); } catch {} }; const changeFolder = async () => { try { const d = await pickSafDir(true); if (d) Alert.alert('تم', 'سيتم حفظ التنزيلات في المجلد الجديد الذي اخترته.'); } catch {} }; if (!material.fileUrl && !material.bodyText) return null; return <View><View style={styles.downloads}><TouchableOpacity style={styles.dlBtn} onPress={saveInApp} disabled={!!busy}><Text style={styles.dlTxt}>{busy === 'app' ? `${pct}%` : 'حفظ داخل التطبيق'}</Text></TouchableOpacity><TouchableOpacity style={[styles.dlBtn, styles.dlBtnAlt]} onPress={saveToDevice} disabled={!!busy}><Text style={[styles.dlTxt, { color: C.brand }]}>{busy === 'device' ? `${pct}%` : 'تنزيل إلى الجهاز'}</Text></TouchableOpacity></View><TouchableOpacity style={styles.shareBtn} onPress={shareLink} activeOpacity={0.85}><Ionicons name="share-social" size={18} color={C.brand} /><Text style={styles.shareTxt}>مشاركة الرابط</Text></TouchableOpacity>{Platform.OS === 'android' && <TouchableOpacity onPress={changeFolder} activeOpacity={0.7} style={{ marginTop: 8, alignSelf: 'center' }}><Text style={{ color: C.muted, fontSize: 12, fontWeight: '700' }}>تغيير مجلد الحفظ</Text></TouchableOpacity>}</View>; }
...
const styles = StyleSheet.create({
  ...
  detailTitle: { fontSize: 24, fontWeight: '900', color: C.brand, textAlign: 'right', alignSelf: 'stretch', width: '100%', writingDirection: 'rtl' }, detailSub: { fontSize: 16, color: C.brand500, marginTop: 4, textAlign: 'right', alignSelf: 'stretch', width: '100%', writingDirection: 'rtl' }, detailPerson: { fontSize: 15, color: C.muted, marginTop: 4, textAlign: 'right', alignSelf: 'stretch', width: '100%', writingDirection: 'rtl' }, image: { width: '100%', height: 260, borderRadius: 16, marginTop: 16, backgroundColor: '#000' }, video: { width: '100%', height: 220, borderRadius: 16, marginTop: 16, backgroundColor: '#000' },
  ...
  full: { backgroundColor: '#12241d', borderRadius: 20, padding: 18, marginTop: 16, alignItems: 'center' }, fullArt: { width: 128, height: 128, borderRadius: 16, backgroundColor: C.brand, alignItems: 'center', justifyContent: 'center', overflow: 'hidden', marginBottom: 12 }, fullArtGlyph: { color: C.gold300, fontSize: 46, fontWeight: '900' }, fullTitle: { color: C.white, fontSize: 17, fontWeight: '900', textAlign: 'center' }, fullPerson: { color: '#aebfb6', fontSize: 13, marginTop: 2, textAlign: 'center' }, seekWrap: { width: '100%', alignSelf: 'stretch', marginTop: 10 }, seekHit: { width: '100%', paddingVertical: 12 }, seekTrack: { height: 8, borderRadius: 4, backgroundColor: '#3f6357', justifyContent: 'center' }, seekFill: { position: 'absolute', height: 8, borderRadius: 4, backgroundColor: C.gold }, seekThumb: { position: 'absolute', width: 20, height: 20, borderRadius: 10, backgroundColor: C.gold, top: -6, borderWidth: 2, borderColor: '#12241d' }, seekTimes: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }, seekTime: { color: '#e6efe9', fontSize: 12, fontWeight: '700' }, fullControls: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 14, marginTop: 10 }, fullSkip: { paddingHorizontal: 6, paddingVertical: 6 }, fullSkipTxt: { color: C.white, fontSize: 15, fontWeight: '800' }, fullPlay: { width: 62, height: 62, borderRadius: 31, backgroundColor: C.gold, alignItems: 'center', justifyContent: 'center' }, fullPlayIcon: { color: C.brand, fontSize: 22, fontWeight: '900' }, fullSpeed: { minWidth: 46, paddingHorizontal: 10, paddingVertical: 8, borderRadius: 12, backgroundColor: '#ffffff18', alignItems: 'center' }, fullSpeedTxt: { color: C.gold300, fontSize: 13, fontWeight: '900' }, fullStopTxt: { color: '#e0a39c', fontSize: 12, fontWeight: '800' },
