...
function SeekBar({ position, duration, onSeek }) {
  ...
  // Standard left-to-right bar: 0 on the LEFT, elapsed on the left, fill grows
  // to the right, drag right = forward — so at 0:00 the thumb sits at the left.
  // The app is forced RTL and THIS Android build mirrors absolute left/right
  // (proven on device: `left:0%` renders at the RIGHT edge) and ignores
  // direction:'ltr'. So under RTL we anchor the fill/thumb with `right` — the
  // platform swaps it back to the visual LEFT — and read the touch physically
  // from the left (locationX). Net: 0 is on the left and the thumb follows the finger.
  const isRTL = I18nManager.isRTL;
  const at = (p) => (isRTL ? { right: `${p}%` } : { left: `${p}%` });
  ...
  return <View style={styles.seekWrap}>
    ...
    >
      <View style={styles.seekTrack}><View style={[styles.seekFill, { ...at(0), width: `${frac * 100}%` }]} /><View style={[styles.seekThumb, at(frac * 100), isRTL ? { marginRight: -10 } : { marginLeft: -10 }]} /></View>
    </View>
...
const styles = StyleSheet.create({
  ...
  detailTitle: { fontSize: 24, fontWeight: '900', color: C.brand, textAlign: 'right', writingDirection: 'rtl', alignSelf: 'stretch', width: '100%' }, detailSub: { fontSize: 16, color: C.brand500, marginTop: 4, textAlign: 'right', writingDirection: 'rtl', alignSelf: 'stretch', width: '100%' }, detailPerson: { fontSize: 15, color: C.muted, marginTop: 4, textAlign: 'right', writingDirection: 'rtl', alignSelf: 'stretch', width: '100%' }, image: { width: '100%', height: 260, borderRadius: 16, marginTop: 16, backgroundColor: '#000' }, video: { width: '100%', height: 220, borderRadius: 16, marginTop: 16, backgroundColor: '#000' },
