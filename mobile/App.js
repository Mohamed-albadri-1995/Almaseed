...
function ArticleHtml({ html }) {
  ...
  const doc = `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><style>html,body{margin:0;padding:0}body{padding:16px;font-family:-apple-system,Roboto,'Segoe UI',sans-serif;font-size:17px;line-height:1.95;color:#20302a;background:#faf7f0;direction:rtl;text-align:right;word-wrap:break-word}img{max-width:100%;height:auto}*{max-width:100%}</style></head><body>${body}<script>function P(){try{window.ReactNativeWebView.postMessage(String(document.body.scrollHeight))}catch(e){}}window.addEventListener('load',P);setTimeout(P,250);setTimeout(P,800);</script></body></html>`;
...
function PipVideo({ item, onExpand, onClose }) {
  const ref = useRef(null); const [playing, setPlaying] = useState(true); const { width: W, height: H } = Dimensions.get('window'); const pan = useRef(new Animated.ValueXY({ x: W - 178, y: H - 220 })).current;
  ...
  const toggle = async () => { try { if (playing) await ref.current?.pauseAsync(); else await ref.current?.playAsync(); } catch {} };
  return <Animated.View style={[styles.pip, { transform: pan.getTranslateTransform() }]} {...responder.panHandlers}><Video ref={ref} source={{ uri: item.url }} resizeMode={ResizeMode.CONTAIN} shouldPlay style={styles.pipVideo} onPlaybackStatusUpdate={(s) => { if (s && typeof s.isPlaying === 'boolean') setPlaying(s.isPlaying); }} onLoad={() => { if (item.positionMillis) ref.current?.setPositionAsync(item.positionMillis); }} /><TouchableOpacity style={styles.pipTap} activeOpacity={0.9} onPress={onExpand} /><TouchableOpacity style={styles.pipPause} onPress={toggle} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}><Text style={styles.pipCtrlIcon}>{playing ? '❚❚' : '▶'}</Text></TouchableOpacity><TouchableOpacity style={styles.pipClose} onPress={onClose} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}><Text style={styles.pipCtrlIcon}>✕</Text></TouchableOpacity></Animated.View>;
}
