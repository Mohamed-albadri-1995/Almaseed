...
const saveMediaToAlbum = async (uri) => { const asset = await MediaLibrary.createAssetAsync(uri); try { const existing = await MediaLibrary.getAlbumAsync(DL_ALBUM); if (existing) await MediaLibrary.addAssetsToAlbumAsync([asset], existing, false); else await MediaLibrary.createAlbumAsync(DL_ALBUM, asset, false); } catch {} return asset; }; /* Documents (pdf/txt/doc) are small; SAF base64 is fine BELOW a size guard, otherwise stream via the share sheet so we never allocate a huge string. */ const saveDoc = async (uri) => { const SAF = FileSystem.StorageAccessFramework; let size = 0; try { const i = await FileSystem.getInfoAsync(uri, { size: true }); size = i.size || 0; } catch {} if (Platform.OS === 'android' && SAF && size > 0 && size < DL_BIG) { try { const perm = await SAF.requestDirectoryPermissionsAsync(); if (perm.granted) { const b64 = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 }); const target = await SAF.createFileAsync(perm.directoryUri, displayBase, MIME[ext] || 'application/octet-stream'); await FileSystem.writeAsStringAsync(target, b64, { encoding: FileSystem.EncodingType.Base64 }); return 'saf'; } } catch {} } if (await Sharing.isAvailableAsync()) { await Sharing.shareAsync(uri); return 'share'; } if (material.fileUrl) { Linking.openURL(material.fileUrl); return 'link'; } return false; }; const saveToDevice = async () => { try { setBusy('device'); setPct(0); /* Only images/videos go to the gallery album. Audio can't be added to the gallery on Android (createAssetAsync/saveToLibraryAsync throw for mp3), which used to drop to the share sheet — so audio (and documents) save to a folder via SAF instead. */ const isGalleryMedia = ['VIDEO', 'IMAGE'].includes(material.fileKind); const uri = await ensureLocal(setPct); let saveUri = uri; try { const pretty = FileSystem.cacheDirectory + `${displayBase}.${ext}`; await FileSystem.deleteAsync(pretty, { idempotent: true }); await FileSystem.copyAsync({ from: uri, to: pretty }); saveUri = pretty; } catch {} if (isGalleryMedia) { const perm = await MediaLibrary.requestPermissionsAsync(false); if (!perm.granted) { Alert.alert('الإذن مطلوب', 'فعّل إذن الوسائط من إعدادات التطبيق لحفظ الملف في جهازك.'); return; } try { await saveMediaToAlbum(saveUri); Alert.alert('تم التنزيل', `حُفظ الملف في مجلد «${DL_ALBUM}» بجهازك (يظهر في المعرض ومدير الملفات).`); } catch (mediaErr) { try { await MediaLibrary.saveToLibraryAsync(saveUri); Alert.alert('تم التنزيل', 'حُفظ الملف في جهازك (المعرض / الموسيقى).'); } catch (e2) { /* Some videos can't be added to the gallery on this device — save to a folder via SAF instead of dropping to the share sheet. */ const r = await saveDoc(saveUri); if (r === 'saf') Alert.alert('تم التنزيل', 'حُفظ الملف في المجلد الذي اخترته.'); else if (r === 'share') Alert.alert('اختر مكان الحفظ', 'هذا الملف كبير — اختر «الملفات» أو تطبيقاً لحفظه في جهازك.'); else if (!r) throw e2; } } } else { const r = await saveDoc(saveUri); if (r === 'saf') Alert.alert('تم التنزيل', 'حُفظ الملف في المجلد الذي اخترته.'); else if (r === 'share') Alert.alert('اختر مكان الحفظ', 'هذا الملف كبير — اختر «الملفات» أو تطبيقاً لحفظه في جهازك.'); else if (!r) Alert.alert('غير متاح', 'تعذّر حفظ هذا النوع على الجهاز.'); } } catch (e) { Alert.alert('تعذّر التنزيل', String(e.message || e)); } finally { setBusy(''); } }; const savePdf = async () => { try { setBusy('pdf'); const stamp = await getEmblemDataUri(); const person = material.author || material.performer || material.speaker || material.host || null; const html = buildArticlePdfHtml(material, stamp, person); const { uri } = await Print.printToFileAsync({ html }); const pretty = FileSystem.cacheDirectory + `${displayBase}.pdf`; await FileSystem.deleteAsync(pretty, { idempotent: true }); await FileSystem.copyAsync({ from: uri, to: pretty }); const SAF = FileSystem.StorageAccessFramework; if (Platform.OS === 'android' && SAF) { const perm = await SAF.requestDirectoryPermissionsAsync(); if (perm.granted) { const b64 = await FileSystem.readAsStringAsync(pretty, { encoding: FileSystem.EncodingType.Base64 }); const target = await SAF.createFileAsync(perm.directoryUri, displayBase, 'application/pdf'); await FileSystem.writeAsStringAsync(target, b64, { encoding: FileSystem.EncodingType.Base64 }); Alert.alert('تم التنزيل', 'حُفظ المقال كملف PDF في المجلد الذي اخترته.'); return; } } if (await Sharing.isAvailableAsync()) { await Sharing.shareAsync(pretty, { mimeType: 'application/pdf', dialogTitle: material.title }); } else { Alert.alert('غير متاح', 'تعذّر حفظ الملف على هذا الجهاز.'); } } catch (e) { Alert.alert('تعذّر إنشاء PDF', String(e.message || e)); } finally { setBusy(''); } };
 const shareLink = async () => { try { await Share.share({ message: `${material.title}\n${API_BASE}/material/${material.id}` }); } catch {} }; if (!material.fileUrl && !material.bodyText) return null; return <View><View style={styles.downloads}><TouchableOpacity style={styles.dlBtn} onPress={saveInApp} disabled={!!busy}><Text style={styles.dlTxt}>{busy === 'app' ? `${pct}%` : 'حفظ داخل التطبيق'}</Text></TouchableOpacity><TouchableOpacity style={[styles.dlBtn, styles.dlBtnAlt]} onPress={saveToDevice} disabled={!!busy}><Text style={[styles.dlTxt, { color: C.brand }]}>{busy === 'device' ? `${pct}%` : 'تنزيل إلى الجهاز'}</Text></TouchableOpacity></View>{!material.fileUrl && !!material.bodyText && <TouchableOpacity style={[styles.dlBtn, styles.dlBtnAlt, { marginTop: 8 }]} onPress={savePdf} disabled={!!busy} activeOpacity={0.85}><Text style={[styles.dlTxt, { color: C.brand }]}>{busy === 'pdf' ? 'جارٍ إنشاء PDF…' : 'تنزيل المقال PDF (بالختم)'}</Text></TouchableOpacity>}<TouchableOpacity style={styles.shareBtn} onPress={shareLink} activeOpacity={0.85}><Ionicons name="share-social" size={18} color={C.brand} /><Text style={styles.shareTxt}>مشاركة الرابط</Text></TouchableOpacity></View>; }
